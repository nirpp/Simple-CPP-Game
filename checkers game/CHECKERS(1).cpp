...
struct team{
				...
				void first(team &b){
					bool x=0,y=0;
					int i=0,idw,idb;//x is for checking it can bite towards right or not..similarly y for left.
					repeat(12){
						...
						repeat(12){int k=0;
							if(c[j].getY()+80==b.c[i].getY()&&c[j].getX()+80==b.c[i].getX())
								...
								if((!v&&c[j].getX()<520)&&c[j].getY()<520) {
									x=1;idw=j;idb=i;cout<<v<<endl;
								...
								if(x){c[idw].move(160,160);b.c[idb].move(1000,1000);return;}
									...
									{bool v=0;
									int k=0;
									repeat(12){if((c[j].getY()+160==b.c[k].getY()&&c[j].getX()-160==b.c[k].getX())||(c[j].getY()+160==c[k].getY()&&c[j].getX()-160==c[k].getX())) {v=1;}k++;
									}
									if((!v&&c[j].getX()>200)&&c[j].getY()<520) {
										y=1;idw=j;idb=i;
										cout<<idw<<"is cutting"<<idb<<endl;
									}
				...
				void initial(  team &p,bool &m){
					...
					repeat(12){
						if(p.c[i].getX()==x1&&p.c[i].getY()==y1)
							{Text t(850,200,"select from your side");
							wait(2);
							t.reset(0,0," ");
							return;}
						i++;   }
					...
					c[id].setFill(true);
						...
						if(s/65536<40||s/65536>680||s%65536<40||s%65536>680){
							Text t(900,200,"click on the board");
							wait(2);
							t.reset(0,0," ");
						...
						if(((p.c[i].getX()==x&&p.c[i].getY()==y)||(c[i].getX()==x&&c[i].getY()==y))||(y>=y1))    //checking overlaping
							{Text t(900,500,"please try playing");wait(2);t.reset(0,0," ");c[id].setColor(COLOR("black"));
						return;}
...
main_program{
	...
	bool q=0,m=1; // m is for surity of player's chance even if he does a wrong move, q is for making the board only
